...
import { useCampaignStore } from '@/api/store/campaignStore/campaign'; // Import the hook
...
const LinkedInSenders: React.FC<LinkedInSendersProps> = ({ selectedAccounts, updateAccounts }) => {
  ...
  // Transform fetched accounts to the display format
  const formatAccountsForDisplay = (accounts: Account[] = []) => {
    return accounts.map(account => ({
      id: account.id,
      name: `${account.firstName} ${account.lastName}`.substring(0, 20) + (account.firstName?.length + account.lastName?.length > 20 ? '...' : ''),
      profilePicture: account.profilePicture || '/placeholder.svg',
